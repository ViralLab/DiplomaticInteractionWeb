...
// TODO: This not a good practice, store this somewhere else
const codeToIdMap = {
    "AFG": 101,
    "ALB": 102,
    "DZA": 103,
...
export class MentionsService {
  ...
  // ============================
  async getInteractionsForCountry(countryCode, options = {}) {
    const {
      limit = 50,
      offset = 0,
      sort = "date",
    ...
    const result = await this.tryIndexedQuery(countryCode);
    if (!result.success) {
    ...
    if (sort === "date") {
      ...
      sortedData.sort((a, b) => {
        const an = (a.reportedName || "");
        const bn = (b.reportedName || "");
        return order === "asc" ? an.localeCompare(bn) : bn.localeCompare(an);
      });
    }
    ...
    return { data: paginated, total, hasMore: offset + limit < total };
  }
  ...
  async tryIndexedQuery(countryCode) {
